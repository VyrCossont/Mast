...
@objc public class DateTimePicker: UIView {
    ...
    public var highlightColor = UIColor(red: 0/255.0, green: 199.0/255.0, blue: 194.0/255.0, alpha: 1) {
        didSet {
            ...
            colonLabel1.textColor = UIColor.white
            colonLabel2.textColor = UIColor.white
...
//            formatter.dateFormat = self.dateFormat
    ...
    public var doneButtonTitle = "Done" {
        didSet {
            doneButton.setTitle("Schedule for \(selectedDateString)", for: .normal)
        }
    }
    ...
    public var completionHandler2: ((String)->Void)?
    ...
    private func configureView() {
        ...
        borderTopView.backgroundColor = darkColor.withAlphaComponent(0)
        ...
        borderBottomView.backgroundColor = darkColor.withAlphaComponent(0)
        ...
        doneButton.topAnchor.constraint(equalTo: contentView.bottomAnchor, constant: 0 - 54 - 26).isActive = true
        doneButton.heightAnchor.constraint(equalToConstant: 54).isActive = true
        ...
        hourTableView.backgroundColor = Colours.tabSelected
        contentView.addSubview(hourTableView)
		
        hourTableView.translatesAutoresizingMaskIntoConstraints = false
        hourTableView.topAnchor.constraint(equalTo: borderBottomView.bottomAnchor, constant: 15).isActive = true
        hourTableView.bottomAnchor.constraint(equalTo: doneButton.topAnchor, constant: -25).isActive = true
        ...
        minuteTableView.backgroundColor = Colours.tabSelected
        contentView.addSubview(minuteTableView)
        
        minuteTableView.translatesAutoresizingMaskIntoConstraints = false
        minuteTableView.topAnchor.constraint(equalTo: borderBottomView.bottomAnchor, constant: 15).isActive = true
        minuteTableView.bottomAnchor.constraint(equalTo: doneButton.topAnchor, constant: -25).isActive = true
        ...
        amPmTableView.backgroundColor = Colours.tabSelected
        contentView.addSubview(amPmTableView)
        
        amPmTableView.translatesAutoresizingMaskIntoConstraints = false
        amPmTableView.topAnchor.constraint(equalTo: borderBottomView.bottomAnchor, constant: -10).isActive = true
        amPmTableView.bottomAnchor.constraint(equalTo: doneButton.topAnchor, constant: 10).isActive = true
    ...
    public func dismissView(sender: UIButton?=nil) {
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0.8, options: .curveLinear, animations: {
            self.layoutIfNeeded()
            NotificationCenter.default.post(name: Notification.Name(rawValue: "doneDate"), object: nil)
        }, completion: { (completed) in
            self.removeFromSuperview()
        })
    }
...
extension DateTimePicker: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if includeMonth {
            ...
            cell.populateItem(date: date, highlightColor: UIColor.white, darkColor: darkColor, locale: locale)
