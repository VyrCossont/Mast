...
class PadDetailViewController2: UIViewController, UITableViewDelegate, UITableViewDataSource, SwipeTableViewCellDelegate, SKPhotoBrowserDelegate, UIViewControllerPreviewingDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(SideDetailCell.self, forCellReuseIdentifier: "cell7")
        self.tableView.register(SideDetailCellImage.self, forCellReuseIdentifier: "cell70")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if self.mainStatus[indexPath.row].reblog?.mediaAttachments.isEmpty ?? self.mainStatus[indexPath.row].mediaAttachments.isEmpty || (UserDefaults.standard.object(forKey: "sensitiveToggle") != nil) && (UserDefaults.standard.object(forKey: "sensitiveToggle") as? Int == 1) {
                let cell = tableView.dequeueReusableCell(withIdentifier: "cell7", for: indexPath) as! SideDetailCell
                ...
                let cell = tableView.dequeueReusableCell(withIdentifier: "cell70", for: indexPath) as! SideDetailCellImage
                cell.configure(self.mainStatus[0])
                cell.profileImageView.tag = indexPath.row
                cell.profileImageView.addTarget(self, action: #selector(self.didTouchProfileM), for: .touchUpInside)
    ...
    @objc func tappedImageDetail(_ sender: UIButton) {
        ...
        if sto[0].reblog?.mediaAttachments[0].type ?? sto[0].mediaAttachments[0].type == .video || sto[0].reblog?.mediaAttachments[0].type ?? sto[0].mediaAttachments[0].type == .gifv {
            ...
            let cell = tableView.cellForRow(at: indexPath) as! SideDetailCellImage
            var images = [SKPhoto]()
            var coun = 0
